...
public class Autonomous extends CommandGroup {
	...
	private void buildDragRaceToCloseScale() {
		...
		addSequential(new SweepingTurn(.7, 1.8, -45 * mult));
		addSequential(new SetTransmissionHigh(false));
		addSequential(new LiftPositionWait(false));
		addSequential(new OpenGripper());
		...
		addSequential(new SweepingTurn(-.6, 2, 90));
	...
	private void buildFarScale() {
		...
		addSequential(new DriveStraight(1.0, 11.15).setAutoShift().setAcceleration(true, false)); // Shift
		...
		addSequential(new SpinExactDegrees(45 * mult));
		addSequential(new LiftPositionWait(false));
		addSequential(new DriveStraight(.7, 1, 2));
		addSequential(new OpenGripper());
